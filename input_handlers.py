...
class EventHandler(BaseEventHandler):
    ...
    def handle_action(self, action: Optional[Action]) -> bool:
        """Handles actions returned from the event methods
...
class AskUserEventHandler(EventHandler):
    """Handles user input for actions that require a special input."""
    ...
    def ev_mousebuttondown(
        ...
        """By default any mouse click exits this user input."""
        return self.on_exit()
...
class LevelUpEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        console.print(x=x + 1, y=1, string="Congratulation! You leveled up!")
        console.print(x=x + 1, y=2, string="Select an attribute to increase.")

        console.print(
            x=x + 1,
            y=4,
            string=f"a) Constituation (+20 HP, from {self.engine.player.fighter.max_hp})",
        )
        console.print(
            x=x + 1,
            y=5,
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        """Render and inventory menu, which displays the item in the inventory, and the letter to select them.
        Will move to a different position depending on where the player is located, so the player can always see where
        they are.
        """
        super().on_render(console)
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        key = event.sym
        if key in MOVE_KEYS:
            modifier = 1  #  Holding modifier keys will speed up the key's movement
...
class SingleRangedAttackHandler(SelectIndexHandler):
    """Handles targeting a single enemy. Only the enemy selected can be affected."""
...
class AreaRangedAttackHandler(SelectIndexHandler):
    """Handles targeting in an area within a given radius. Any entity in the area will also be affected."""
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        # Draw a rectangle around the area, so the player can see the effected area.
        console.draw_frame(
            x=x - self.radius - 1,
            y=y - self.radius - 1,
            width=self.radius**2,
...
class MainGameEventHandler(EventHandler):
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if key == tcod.event.K_d and modifier & (

...
class FireballDamageConsumeable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if not targets_hit:
            raise Impossible("There are no targets to hit in the radius.")
        self.consume()
...
class LightningDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if target:
            self.engine.message_log.add_message(
                f"A lightning bolt strikes {target.name} with a loud thunder, for {self.damage} damage!"
            )
